...
- **FR-014**: Project MUST include Architecture Decision Records that, at minimum, document: (a) the choice of a custom retrieval pipeline over an orchestration framework, (b) the choice of backend language and web framework (Python + FastAPI, with Node.js alternatives such as NestJS and Fastify considered), (c) the choice of vector store, (d) the choice of PDF and DOCX parsers, (e) the strategy for swappable AI/embedding providers, and (f) the choice of streaming approach. Each ADR MUST state context, decision, alternatives considered, and consequences.
...
- **SC-007**: A reviewer browsing the project's decision records can find one record per major design choice (architecture style, backend language/framework, vector store, parsers, provider strategy, streaming approach), each with context / decision / alternatives / consequences clearly separated.
