...
- [x] T023 [P] [US1] Unit test `tests/unit/test_chunker.py`: ≤ 500 tokens per chunk asserted by re-encoding emitted text with `tiktoken cl100k_base`; adjacent chunks share their last `overlap_tokens` (token-id comparison); deterministic sequence chunk_ids; locator inheritance from dominant segment; constructor validation; tokenizer char-span round-trip on ASCII.
