...
- [X] T038 [P] [US2] Implement `frontend/src/api/ask.ts`: `streamAnswer(req: AskRequest, onEvent: (e: SseEvent) => void, signal: AbortSignal): Promise<void>`. `fetch` POST with `Accept: text/event-stream`, body JSON; reads `response.body!.getReader()`, passes chunks to `sse/parser.ts`. On `signal.aborted` propagates `AbortError` to caller. (Request body currently typed as `unknown` instead of the OpenAPI `AskRequest`; tighten once T015's generated types are wired up.)
